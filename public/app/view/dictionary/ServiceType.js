...
Ext.application({
    ...
    launch: function(){
        ...
        var store_service_type = Ext.create('Ext.data.Store', {
            storeId: 'store_service_type'+page,
            ...
            proxy: {
                type: 'ajax',
                url: 'getServiceType',//action=getInc
                method: 'GET',
                reader: {
                    type: 'json',
        ...
        store_service_type.filter(filters);
        store_service_type.proxy.extraParams = {
            // filter: Ext.encode(filters),
            action :'getEntity'
        };

        store_service_type.reload({
            params:{
                start:0,
                limit:25
            }
        ...
        var editor_grid_service_type =  Ext.create('Ext.grid.plugin.RowEditing', {
            licksToMoveEditor: 1,
            autoCancel: true,
        ...
        var grid_service_type = Ext.create('Ext.grid.Panel', {
            title: 'List '+title,
            border:false,
            region: 'center',
            ...
            tbar:[
                {
                    ...
                    handler: function() {
                        ...
                        var order = 1 ;
                        var index = 0 ;
                        store_service_type.each(function(r,i){
                            data.push(r.data);
                            order++ ;
                            index++ ;
                        });
                        // Create a model instance
                        var r = Ext.create('model_service_type', {
                            flag :'Insert' ,
                        ...
                        store_service_type.insert(index, r);
                        editor_grid_service_type.startEdit(r, 0);
                ...
                },
                {
                    ...
                    handler: function() {
                        var records = grid_service_type.getSelectionModel().getSelection()[0];
                        if (records) {
                            Ext.MessageBox.show({
                                title:'Warning',
        ...
        editor_grid_service_type.on({
            scope: this,
            beforeedit: function(roweditor, context) {

            },
            afteredit: function(roweditor, context) {
                var row = grid_service_type.getSelectionModel().getSelection()[0];
                var data = [] ;
            ...
            canceledit : function(){
                if (grid_service_type.getSelectionModel().hasSelection()) {
                    var row = grid_service_type.getSelectionModel().getSelection()[0];
                }
            }
        });
