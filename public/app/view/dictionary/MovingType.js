...
Ext.application({
    ...
    launch: function(){
        ...
        var model_moving_type = Ext.define('model_moving_type', {
            extend: 'Ext.data.Model',
            fields: ['entity_name','code', 'description']
        });

        var store_moving_type = Ext.create('Ext.data.Store', {
            storeId: 'store_moving_type'+page,
            model: model_moving_type,
            remoteFilter:true,
            remoteSort:true,
            autoLoad: false,
            proxy: {
        ...
        store_moving_type.filter(filters);
        store_moving_type.proxy.extraParams = {
            // filter: Ext.encode(filters),
            action :'getEntity'
        };

        store_moving_type.reload({
            params:{
                start:0,
                limit:25
            }
        ...
        var editor_grid_moving_type =  Ext.create('Ext.grid.plugin.RowEditing', {
            licksToMoveEditor: 1,
            autoCancel: true,
        ...
        var grid_moving_type = Ext.create('Ext.grid.Panel', {
            title: 'List '+title,
            border:false,
            region: 'center',
            ...
            tbar:[
                {
                    ...
                    handler: function() {
                        ...
                        var order = 1 ;
                        var index = 0 ;
                        store_moving_type.each(function(r,i){
                            data.push(r.data);
                            order++ ;
                            index++ ;
                        });
                        // Create a model instance
                        var r = Ext.create('model_moving_type', {
                            flag :'Insert' ,
                        ...
                        store_moving_type.insert(index, r);
                        editor_grid_moving_type.startEdit(r, 0);
                ...
                },
                {
                    ...
                    handler: function() {
                        var records = grid_moving_type.getSelectionModel().getSelection()[0];
                        if (records) {
                            Ext.MessageBox.show({
                                title:'Warning',
        ...
        editor_grid_moving_type.on({
            scope: this,
            beforeedit: function(roweditor, context) {

            },
            afteredit: function(roweditor, context) {
                var row = grid_moving_type.getSelectionModel().getSelection()[0];
                var data = [] ;
            ...
            canceledit : function(){
                if (grid_moving_type.getSelectionModel().hasSelection()) {
                    var row = grid_moving_type.getSelectionModel().getSelection()[0];
                }
            }
        });
