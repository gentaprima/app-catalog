...
Ext.application({
    ...
    launch: function(){
        ...
        var store_material_category = Ext.create('Ext.data.Store', {
            storeId: 'store_material_category'+page,
        ...
        store_material_category.filter(filters);
        store_material_category.proxy.extraParams = {
            // filter: Ext.encode(filters),
            action :'getEntity'
        };

        store_material_category.reload({
            params:{
                start:0,
                limit:25
            }
        ...
        var editor_grid_material_category =  Ext.create('Ext.grid.plugin.RowEditing', {
            licksToMoveEditor: 1,
            autoCancel: true,
        ...
        var grid_material_category = Ext.create('Ext.grid.Panel', {
            //title: title,
            border:false,
            region: 'center',
            ...
            tbar:[
                {
                    ...
                    handler: function() {
                        ...
                        var order = 1 ;
                        var index = 0 ;
                        store_material_category.each(function(r,i){
                            data.push(r.data);
                            order++ ;
                            index++ ;
                        });
                        // Create a model instance
                        var r = Ext.create('model_material_category', {
                            flag :'Insert' ,
                        ...
                        store_material_category.insert(index, r);
                        editor_grid_material_category.startEdit(r, 0);
                ...
                },
                {
                    ...
                    handler: function() {
                        var records = grid_material_category.getSelectionModel().getSelection()[0];
                        if (records) {
                            Ext.MessageBox.show({
                                title:'Warning',
        ...
        editor_grid_material_category.on({
            scope: this,
            beforeedit: function(roweditor, context) {

            },
            afteredit: function(roweditor, context) {
                var row = grid_material_category.getSelectionModel().getSelection()[0];
                var data = [] ;
            ...
            canceledit : function(){
                if (grid_characteristicsM.getSelectionModel().hasSelection()) {
                    var row = grid_characteristicsM.getSelectionModel().getSelection()[0];
                }
            }
        });
        ...
        var store_service_category = Ext.create('Ext.data.Store', {
            storeId: 'store_service_category'+page,
        ...
        filters.push(entity_name['initialConfig']) ;
        store_service_category.filter(filters);
        store_service_category.proxy.extraParams = {
            // filter: Ext.encode(filters),
            action :'getEntity'
        };

        store_service_category.reload({
            params:{
                start:0,
                limit:25
            }
        ...
        // Bikin Cart Product Item di dalam Grid
        var editor_grid_service_category =  Ext.create('Ext.grid.plugin.RowEditing', {
            licksToMoveEditor: 1,
            autoCancel: true,
            saveBtnText    : "Update",//My Save Button Text&quot;,
            saveBtnIcon: 'add-data',
        ...
        var grid_service_category = Ext.create('Ext.grid.Panel', {
            //title: title,
            border:false,
            region: 'center',
            ...
            tbar:[
                {
                    ...
                    handler: function() {
                        ...
                        var order = 1 ;
                        var index = 0 ;
                        store_service_category.each(function(r,i){
                            data.push(r.data);
                            order++ ;
                            index++ ;
                        });
                        // Create a model instance
                        var r = Ext.create('model_service_category', {
                            flag :'Insert' ,
                            // entity_name:'servicecategory'
                            // sequence : sequence,
                            // inc : Ext.getCmp('detail_inc'+page).getValue() ,
                            // type: 'O',
                        });

                        store_service_category.insert(index, r);
                        editor_grid_service_category.startEdit(r, 0);
...

                {
                    ...
                    disabled:!ROLE.RemoveDictionary,
                    handler: function() {
                        var records = grid_service_category.getSelectionModel().getSelection()[0];
                        if (records) {
                            Ext.MessageBox.show({
                                title:'Warning',
                                msg:'Sure to delete this data?',
        ...
        editor_grid_service_category.on({
            scope: this,
            beforeedit: function(roweditor, context) {

            },
            afteredit: function(roweditor, context) {
                var row = grid_service_category.getSelectionModel().getSelection()[0];
                var data = [] ;
                data.push(Ext.encode(row.data));
                Ext.MessageBox.show({
                    title: 'Please wait',
...
/*                             dockedItems: [
                                ...
                                {
                                    xtype: 'header',
                                    style: ' backgroundColor:#FFFFFF; backgroundImage:none;text-align: left;font-color:white;font-weight: bold;font-size:25px;font-family: "Comic Sans MS", cursive, sans-serif',
                                    html: 'List '+title,
