...
Ext.application({
    ...
    launch: function(){
        ...
        var editor_grid_currency =  Ext.create('Ext.grid.plugin.RowEditing', {
            licksToMoveEditor: 1,
            autoCancel: true,
        ...
        var grid_currency = Ext.create('Ext.grid.Panel', {
            title: 'List '+title,
            border:false,
            region: 'center',
            ...
            tbar:[
                {
                    ...
                    handler: function() {
                        ...
                        var order = 1 ;
                        var index = 0 ;
                        store_currency.each(function(r,i){
                            data.push(r.data);
                            order++ ;
                            index++ ;
                        });
                        // Create a model instance
                        var r = Ext.create('model_currency', {
                            flag :'Insert' ,
                        ...
                        store_currency.insert(index, r);
                        editor_grid_currency.startEdit(r, 0);
                ...
                },
                {
                    ...
                    handler: function() {
                        var records = grid_currency.getSelectionModel().getSelection()[0];
                        if (records) {
                            Ext.MessageBox.show({
                                title:'Warning',
        ...
        editor_grid_currency.on({
            scope: this,
            beforeedit: function(roweditor, context) {

            },
            afteredit: function(roweditor, context) {
                var row = grid_currency.getSelectionModel().getSelection()[0];
                var data = [] ;
            ...
            canceledit : function(){
                if (grid_currency.getSelectionModel().hasSelection()) {
                    var row = grid_currency.getSelectionModel().getSelection()[0];
                }
            }
        });
