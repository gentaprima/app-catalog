...
Ext.application({
    ...
    launch: function(){
        ...
        var model_hazard_type = Ext.define('model_hazard_type', {
            extend: 'Ext.data.Model',
            fields: ['entity_name','code', 'description']
        });

        var store_hazard_type = Ext.create('Ext.data.Store', {
            storeId: 'store_hazard_type'+page,
            model: model_hazard_type,
            remoteFilter:true,
            remoteSort:true,
            autoLoad: false,
            proxy: {
                type: 'ajax',
                url: 'getHazardType',//action=getInc
                method: 'GET',
                reader: {
                    type: 'json',
        ...
        store_hazard_type.filter(filters);
        store_hazard_type.proxy.extraParams = {
            // filter: Ext.encode(filters),
            action :'getEntity'
        };

        store_hazard_type.reload({
            params:{
                start:0,
                limit:25
            }
        ...
        var editor_grid_hazard_type =  Ext.create('Ext.grid.plugin.RowEditing', {
            licksToMoveEditor: 1,
            autoCancel: true,
        ...
        var grid_hazard_type = Ext.create('Ext.grid.Panel', {
            title: 'List '+title,
            border:false,
            region: 'center',
            store: store_hazard_type,
            frame:true,
            columns: [
                {
                    text: 'Code',
            ...
            tbar:[
                {
                    ...
                    handler: function() {
                        ...
                        var order = 1 ;
                        var index = 0 ;
                        store_hazard_type.each(function(r,i){
                            data.push(r.data);
                            order++ ;
                            index++ ;
                        });
                        // Create a model instance
                        var r = Ext.create('model_hazard_type', {
                            flag :'Insert' ,
                        ...
                        store_hazard_type.insert(index, r);
                        editor_grid_hazard_type.startEdit(r, 0);
                ...
                },
                {
                    ...
                    handler: function() {
                        var records = grid_hazard_type.getSelectionModel().getSelection()[0];
                        if (records) {
                            Ext.MessageBox.show({
                                title:'Warning',
        ...
        editor_grid_hazard_type.on({
            scope: this,
            beforeedit: function(roweditor, context) {

            },
            afteredit: function(roweditor, context) {
                var row = grid_hazard_type.getSelectionModel().getSelection()[0];
                var data = [] ;
            ...
            canceledit : function(){
                if (grid_hazard_type.getSelectionModel().hasSelection()) {
                    var row = grid_hazard_type.getSelectionModel().getSelection()[0];
                }
            }
        });
