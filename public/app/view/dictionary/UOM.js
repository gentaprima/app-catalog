...
Ext.application({
    ...
    launch: function(){
        ...
        var model_UOM = Ext.define('model_UOM', {
            extend: 'Ext.data.Model',
            fields: ['entity_name','code', 'description']
        });

        var store_UOM = Ext.create('Ext.data.Store', {
            storeId: 'store_UOM'+page,
            model: model_UOM,
            remoteFilter:true,
            remoteSort:true,
            autoLoad: false,
        ...
        store_UOM.filter(filters);
        store_UOM.proxy.extraParams = {
        ...
        var editor_grid_UOM =  Ext.create('Ext.grid.plugin.RowEditing', {
            licksToMoveEditor: 1,
            autoCancel: true,
        ...
        var grid_UOM = Ext.create('Ext.grid.Panel', {
            title: 'List '+ title,
            border:false,
            region: 'center',
            store: store_UOM,
            ...
            tbar:[
                {
                    ...
                    handler: function() {
                        ...
                        var order = 1 ;
                        var index = 0 ;
                        store_UOM.each(function(r,i){
                            data.push(r.data);
                            order++ ;
                            index++ ;
                        });
                        // Create a model instance
                        var r = Ext.create('model_UOM', {
                            flag :'Insert' ,
                        ...
                        store_UOM.insert(index, r);
                        editor_grid_UOM.startEdit(r, 0);
                ...
                },
                {
                    ...
                    handler: function() {
                        var records = grid_UOM.getSelectionModel().getSelection()[0];
                        if (records) {
                            Ext.MessageBox.show({
                                title:'Warning',
        ...
        editor_grid_UOM.on({
            ...
            afteredit: function(roweditor, context) {
                var row = grid_UOM.getSelectionModel().getSelection()[0];
                var data = [] ;
            ...
            canceledit : function(){
                if (grid_UOM.getSelectionModel().hasSelection()) {
                    var row = grid_UOM.getSelectionModel().getSelection()[0];
                }
            }
        });
