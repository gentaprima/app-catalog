...
Ext.application({
    ...
    launch: function(){
        ...
        var store_material_type = Ext.create('Ext.data.Store', {
            storeId: 'store_material_type'+page,
            ...
            proxy: {
                type: 'ajax',
                url: 'getMaterialType',//action=getInc
                method: 'GET',
                reader: {
                    type: 'json',
        ...
        store_material_type.filter(filters);
        store_material_type.proxy.extraParams = {
            // filter: Ext.encode(filters),
            action :'getEntity'
        };

        store_material_type.reload({
            params:{
                start:0,
                limit:25
            }
        ...
        var editor_grid_material_type =  Ext.create('Ext.grid.plugin.RowEditing', {
            licksToMoveEditor: 1,
            autoCancel: true,
        ...
        var grid_material_type = Ext.create('Ext.grid.Panel', {
            title: 'List '+title,
            border:false,
            region: 'center',
            ...
            tbar:[
                {
                    ...
                    handler: function() {
                        ...
                        var order = 1 ;
                        var index = 0 ;
                        store_material_type.each(function(r,i){
                            data.push(r.data);
                            order++ ;
                            index++ ;
                        });
                        // Create a model instance
                        var r = Ext.create('model_material_type', {
                            flag :'Insert' ,
                        ...
                        store_material_type.insert(index, r);
                        editor_grid_material_type.startEdit(r, 0);
                ...
                },
                {
                    ...
                    handler: function() {
                        var records = grid_material_type.getSelectionModel().getSelection()[0];
                        if (records) {
                            Ext.MessageBox.show({
                                title:'Warning',
        ...
        editor_grid_material_type.on({
            scope: this,
            beforeedit: function(roweditor, context) {

            },
            afteredit: function(roweditor, context) {
                var row = grid_material_type.getSelectionModel().getSelection()[0];
                var data = [] ;
            ...
            canceledit : function(){
                if (grid_material_type.getSelectionModel().hasSelection()) {
                    var row = grid_material_type.getSelectionModel().getSelection()[0];
                }
            }
        });
