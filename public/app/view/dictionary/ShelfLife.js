...
Ext.application({
    ...
    launch: function(){
        ...
        var model_shelf_life = Ext.define('model_shelf_life', {
            extend: 'Ext.data.Model',
            fields: ['entity_name','code', 'description']
        });

        var store_shelf_life = Ext.create('Ext.data.Store', {
            storeId: 'store_shelf_life'+page,
            model: model_shelf_life,
            remoteFilter:true,
            remoteSort:true,
            autoLoad: false,
            proxy: {
                type: 'ajax',
                url: 'getCurrency',//action=getInc
                method: 'GET',
                reader: {
                    type: 'json',
        ...
        store_shelf_life.filter(filters);
        store_shelf_life.proxy.extraParams = {
            // filter: Ext.encode(filters),
            action :'getEntity'
        };

        store_shelf_life.reload({
            params:{
                start:0,
                limit:25
            }
        ...
        var editor_grid_shelf_life =  Ext.create('Ext.grid.plugin.RowEditing', {
            licksToMoveEditor: 1,
            autoCancel: true,
        ...
        var grid_shelf_life = Ext.create('Ext.grid.Panel', {
            title: 'List '+title,
            border:false,
            region: 'center',
            store: store_shelf_life,
            frame:true,
            columns: [
                {
                    text: 'Code',
            ...
            tbar:[
                {
                    ...
                    handler: function() {
                        ...
                        var order = 1 ;
                        var index = 0 ;
                        store_shelf_life.each(function(r,i){
                            data.push(r.data);
                            order++ ;
                            index++ ;
                        });
                        // Create a model instance
                        var r = Ext.create('model_shelf_life', {
                            flag :'Insert' ,
                        ...
                        store_shelf_life.insert(index, r);
                        editor_grid_shelf_life.startEdit(r, 0);
                ...
                },
                {
                    ...
                    handler: function() {
                        var records = grid_shelf_life.getSelectionModel().getSelection()[0];
                        if (records) {
                            Ext.MessageBox.show({
                                title:'Warning',
        ...
        editor_grid_shelf_life.on({
            scope: this,
            beforeedit: function(roweditor, context) {

            },
            afteredit: function(roweditor, context) {
                var row = grid_shelf_life.getSelectionModel().getSelection()[0];
                var data = [] ;
            ...
            canceledit : function(){
                if (grid_shelf_life.getSelectionModel().hasSelection()) {
                    var row = grid_shelf_life.getSelectionModel().getSelection()[0];
                }
            }
        });
