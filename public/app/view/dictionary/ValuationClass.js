...
Ext.application({
    ...
    launch: function(){
        var model_valuation_class = Ext.define('model_valuation_class', {
            extend: 'Ext.data.Model',
            fields: ['companies_m_id','valuation', 'valuationdesc']
        });

        var store_valuation_class = Ext.create('Ext.data.Store', {
            storeId: 'store_valuation_class'+page,
            model: model_valuation_class,
            remoteFilter:true,
            remoteSort:true,
            autoLoad: false,
            proxy: {
                type: 'ajax',
                url: '/getValuationClass',//action=getInc
                method: 'GET',
                reader: {
                    type: 'json',
...
// Bikin Cart Product Item di dalam Grid
        var editor_grid_valuation_class =  Ext.create('Ext.grid.plugin.RowEditing', {
            licksToMoveEditor: 1,
            autoCancel: true,
        ...
        editor_grid_valuation_class.on({
            scope: this,
            beforeedit: function(roweditor, context) {

            },
            afteredit: function(roweditor, context) {
                var row = grid_valuation_class.getSelectionModel().getSelection()[0];
                var data = [] ;
        ...
        var grid_valuation_class = Ext.create('Ext.grid.Panel', {
            title: 'List '+title,
            border:false,
            region: 'center',
            ...
            columns: [
                ...
                },
                {
                    ...
                    editor:{
                        xtype: 'textfield',
                        readOnly:true,
                        disabled:!ROLE.EditValuationClass,
            ...
            tbar:[
                {
                    ...
                    text: 'Add',
                    iconCls: 'add-data',
                    disabled:!ROLE.AddValuationClass,
                    handler: function() {
                        var data = [];
                        var order = 1 ;
                        var index = 0 ;
                        store_valuation_class.each(function(r,i){
                            data.push(r.data);
                            order++ ;
                            index++ ;
                        });
                        // Create a model instance
                        var r = Ext.create('model_valuation_class', {
                            flag :'Insert' ,
                            // sequence : sequence,
                        ...
                        store_valuation_class.insert(index, r);
                        editor_grid_valuation_class.startEdit(r, 0);
                ...
                },
                {
                    ...
                    text: 'Remove',
                    iconCls: 'row-delete',
                    disabled:!ROLE.RemoveValuationClass,
                    handler: function() {
                        var records = grid_valuation_class.getSelectionModel().getSelection()[0];
                        if (records) {
                            // console.log(records);
                            Ext.MessageBox.show({
                                title:'Warning',
            ...
            listeners: {
                ...
                beforeedit: function(plugin, context) {
					return (ROLE.EditValuationClass ? true : false);
                }
            }
        });
