...
Ext.application({
    ...
    launch: function(){
        ...
        var model_stock_type = Ext.define('model_stock_type', {
            extend: 'Ext.data.Model',
            fields: ['entity_name','code', 'description']
        });

        var store_stock_type = Ext.create('Ext.data.Store', {
            storeId: 'store_stock_type'+page,
            model: model_stock_type,
            remoteFilter:true,
            remoteSort:true,
            autoLoad: false,
            proxy: {
        ...
        store_stock_type.filter(filters);
        ...
        var editor_grid_stock_type =  Ext.create('Ext.grid.plugin.RowEditing', {
            licksToMoveEditor: 1,
            autoCancel: true,
        ...
        var grid_stock_type = Ext.create('Ext.grid.Panel', {
            title: 'List '+title,
            border:false,
            region: 'center',
            ...
            tbar:[
                {
                    ...
                    handler: function() {
                        ...
                        var order = 1 ;
                        var index = 0 ;
                        store_stock_type.each(function(r,i){
                            data.push(r.data);
                            order++ ;
                            index++ ;
                        });
                        // Create a model instance
                        var r = Ext.create('model_stock_type', {
                            flag :'Insert' ,
                        ...
                        store_stock_type.insert(index, r);
                        editor_grid_stock_type.startEdit(r, 0);
                ...
                },
                {
                    ...
                    handler: function() {
                        var records = grid_stock_type.getSelectionModel().getSelection()[0];
                        if (records) {
                            Ext.MessageBox.show({
                                title:'Warning',
        ...
        editor_grid_stock_type.on({
            scope: this,
            beforeedit: function(roweditor, context) {

            },
            afteredit: function(roweditor, context) {
                var row = grid_stock_type.getSelectionModel().getSelection()[0];
                var data = [] ;
            ...
            canceledit : function(){
                if (grid_stock_type.getSelectionModel().hasSelection()) {
                    var row = grid_stock_type.getSelectionModel().getSelection()[0];
                }
            }
        });
