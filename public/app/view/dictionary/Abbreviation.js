...
Ext.application({
    ...
    launch: function(){
        ...
        var model_ABBR = Ext.define('model_ABBR', {
            extend: 'Ext.data.Model',
            fields: ['entity_name','code', 'description']
        });

        var store_ABBR = Ext.create('Ext.data.Store', {
            storeId: 'store_ABBR'+page,
            model: model_ABBR,
            remoteFilter:true,
            remoteSort:true,
            autoLoad: false,
            proxy: {
                type: 'ajax',
                url: 'getAbbreviation',//action=getInc
                method: 'GET',
                reader: {
                    type: 'json',
        ...
        store_ABBR.filter(filters);
        store_ABBR.proxy.extraParams = {
        ...
        var editor_grid_ABBR =  Ext.create('Ext.grid.plugin.RowEditing', {
            licksToMoveEditor: 1,
            autoCancel: true,
        ...
        var grid_ABBR = Ext.create('Ext.grid.Panel', {
            title: 'List '+title,
            border:false,
            region: 'center',
            store: store_ABBR,
            ...
            tbar:[
                {
                  ...
                  // disabled:!ROLE.AddDictionary,
                    handler: function() {
                        ...
                        var order = 1 ;
                        var index = 0 ;
                        store_ABBR.each(function(r,i){
                            data.push(r.data);
                            order++ ;
                            index++ ;
                        });
                        // Create a model instance
                        var r = Ext.create('model_ABBR', {
                            flag :'Insert' ,
                        ...
                        store_ABBR.insert(index, r);
                        editor_grid_ABBR.startEdit(r, 0);
                ...
                },
                {
                    ...
                    handler: function() {
                        var records = grid_ABBR.getSelectionModel().getSelection()[0];
                        if (records) {
                            Ext.MessageBox.show({
                                title:'Warning',
        ...
        editor_grid_ABBR.on({
            ...
            afteredit: function(roweditor, context) {
                var row = grid_ABBR.getSelectionModel().getSelection()[0];
                var data = [] ;
            ...
            canceledit : function(){
                if (grid_ABBR.getSelectionModel().hasSelection()) {
                    var row = grid_ABBR.getSelectionModel().getSelection()[0];
                }
            }
        });
