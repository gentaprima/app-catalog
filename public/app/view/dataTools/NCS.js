ROLE = Ext.decode('{"EditMGC":true,"Cataloguer":true,"ApplyChangeCrossRefrence":true,"ApplyChangeCharacteristic":true,"EditColloquialName":true,"RemoveColloquialName":true,"AddColloquialName":true,"ChangeShortName":true,"UploadImages":true,"EditCharacteristic":true,"RemoveCharacteristic":true,"AddCharacteristic":true,"RemoveImages":true,"EditShortNameCode":true,"IncOptional":true,"RemoveIncMaterial":true,"AddIncMaterial":true}');
valid_script = true;
...
//            fields: ['inc','mrcode', 'characteristics', 'nvalue']
        });
...
//COMBOBOX GROUPCLASS
        ...
        var store_mgc_m = Ext.create('Ext.data.Store', {
            id: 'store_mgc_m'+page,
            model: model_groupclass_m,
            groupField: 'group_header',
            proxy: {
        ...
        var grid_inc_m = Ext.create('Ext.grid.Panel', {
            ...
            tbar:[
                {
                    ...
                    text: 'Add Inc',
                    iconCls: 'add-data',
                    disabled: !ROLE.AddIncMaterial,
                    id: 'add_inc'+page,
                    hidden:!ROLE.AddIncMaterial,
                    handler: function() {
            ...
            listeners: {
                select: function(selModel, record, index, options){
					...
					Ext.getCmp('detail_decription1'+page).setValue(record.data.description);
                    // INC
                        ...
                        callback : function(records, operation, success) {
                            if (success) {
                                ...
                                if (store_mgc_list.getCount() > 0) {
									...
									Ext.getCmp('mgc_list1'+page).setValue(mgcList);
                        ...
                        },
                        {
                            ...
                            /*listConfig: {
                              tpl: Ext.create('Ext.XTemplate',
                                      ...
                                      '</tpl>',
                                      '<li role="option" class="x-boundlist-item" style="padding-left: 12px">{group_class_name}</li>',
                                      '</tpl>' +
                                      '</ul>',
                                      {
        ...
        var store_inc_characteristics = new Ext.data.JsonStore({
                ...
                afteredit: function(roweditor, context) {
                 /*   Ext.MessageBox.show({
					...
					*/
                    ...
                    var data = [] ;
                    data.push(Ext.encode(row.data));
				//	alert('data push 1191 row kw : '+row.data['sequence']);
				//	alert('data push 1192 row kw : '+row.data['id']);
					alert('data push 1193 row kw : '+row.data['inc']);
                    Ext.Ajax.request({
                        scope:this,
                        url: '/SaveIncCharacteristics',
						//  url: '/MoveIncC/'+row.data['inc'],
                        method: 'POST',
                        dataType: 'json',
                        params:{
                            _token : csrf_token,
                        ...
                        success:function(response, request){
							...
							alert('data push 1206 row kw : '+row.data['inc']);
                            var filters = [];
                            var inc_filter = new Ext.util.Filter({
                                operator: 'eq',
																					...
																					buttons: [
																						{
																							...
																							hidden:!ROLE.EditMGC,
																							handler: function() {
																								var enc = Ext.getCmp('detail_inc'+page).getValue();
																								Ext.getCmp('formIncMx'+page).form.submit({
																									url: 'SaveMCG',
																									method: 'POST',
																									dataType: 'json',
																									params:{
																									...
																									success:function(response, request){
																										...
																										Ext.getCmp('formIncMx'+page).reset();
																										winIncM.hide();
																										// store_inc_images.reload();
																										// winDetailIncImages.hide();
																										// Ext.getCmp('inc_images_description'+page).reset();
