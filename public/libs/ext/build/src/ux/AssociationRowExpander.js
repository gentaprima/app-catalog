Ext.define('Ux.grid.plugin.AssociationRowExpander', {
    ...
    createCmp : function(record, id, config) {
		...
		var store_d = Ext.create('Ext.data.JsonStore', {
			...
			proxy: {
				 type: 'ajax',
				 url: ajax_url_expander ,//'ajax.handler.php?id=numberid.5',//ajax_url ,//+ extraparams ,  // url that will load data with respect to start and limit params
				 method: 'GET',
				 reader: {
					 type: 'json',
					 root: 'data',
        ...
        if (type === 'hasMany') {
            ...
            store_d.proxy.extraParams.expander_id= record.data.id;
			store_d.load({
				params:{
					start:0,
					limit: 500,
