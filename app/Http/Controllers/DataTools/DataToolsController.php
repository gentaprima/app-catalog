...
class DataToolsController extends Controller
{
    ...
    function SaveIncCharacteristicsNew(Request $request){
        $validate = Validator::make($request->all(),[
            'inc_m_id' => "required",
            'inc' => "required",
            'characteristics_m_id' => "required",
        ...
        if($request->id != null){
            // update
            $incCharacteristic = IncCharacteristics::find($request->id);
            $incCharacteristic->inc_m_id = $request->inc_m_id;
            $incCharacteristic->inc = $request->inc;
            $incCharacteristic->characteristics_m_id = $request->characteristics_m_id;
            ...
            //insert
            $getIncMaxSequence = DB::table("inc_characteristics")
            ...
            foreach ($incCharacteristicMrow as $arrRow) {
                if ($arrRow['id'] !== "last_insert_id") {
                    if ($arrRow['sequence_old'] !== $arrRow['sequence']) {
                        ...
                        DB::enableQueryLog();
                        //  $seqx =  increment('sequence');
                        //  echo $seqx;
                        //  $seq1 = $arrRow['sequence_old'];
                        //  $seq2 = $arrRow['sequence'];
                        //   $id = $arrRow['id'];
                        //   $seq1 = $arrRow['inc'];
                        //   $sql = "CALL move_sequence_inc1(:seq1,:id,:seq2,:inc)" ;
                        //   echo $sql;
                        //  $result = BaseModel::buildSql($sql,['seq1' => $seq1,'id' => $id,'seq2' => $seq2,'inc' => $inc ]);;	
                        //   return Response::json($result,200);	
                        ...
                        //    IncCharacteristics::where('incx','=',$arrRow['inc'])
                        //	     ->where('sequence','>',$arrRow['sequence_old'])
                        //       ->update(['sequence' =>  'sequence-1' ,'type'=>$arrRow['type'] ]);

                        $this->assertSessionHasErrors(['sequence', 'sequence_old']);
    ...
    public function SaveMCG(REQUEST $request)
    {
        DB::beginTransaction();
        try {
            $input = Input::all();
